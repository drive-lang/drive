module Prog
	class Error_Formatter
		...
		def error_name_styled
			Prog::Ascii.bold(Prog::Ascii.red(error.class.name.split('::').last))
		end
		...
		def location_line
			...
			if expression.is_a?(Prog::Expression) && expression.l0
		...
		def source_snippet
			...
			lines       = Backend::Interpreter.cached_source_by_filename[source_file] || []
			...
			(start_line..end_line).each do |line_num|
				...
				prefix         = Prog::Ascii.cyan("#{line_num.to_s.rjust(5)} │ ")
				...
				if is_error_line
					...
					styled_span = Prog::Ascii.bold(Prog::Ascii.red(error_span))

					# Use Colors.make only for the single-line case where we want a different style
					if l0 == l1 && line_num == l0
						styled_span = Prog::Ascii.bold(Prog::Ascii.make(error_span))
					end
					...
					prefix     = Prog::Ascii.cyan("#{' '.rjust(5)} │ ")
					error_line = spaces + Prog::Ascii.bold(Prog::Ascii.red("╰── " + error_label))
