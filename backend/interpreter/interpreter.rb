...
module Backend
	class Interpreter
		...
		# Raw source of runtime assets injected into every matching response (dom.js, view_transition.css) -- not Backend source, so it never goes through the lex/parse caches above. Kept class-level for the same reason: identical for every Interpreter in the process, so re-reading from prog on every request is pure waste.
		...
		class << self
			...
			# backend/shared/cached_by_path.rb), so this just delegates to each one's own generic reset.
			def reset_file_caches! paths = nil
				reset_cached_by_path! paths
				Backend::Declarator.reset_cached_by_path! paths
			end
		end
		...
		def initialize
			@dom_input_elements            = {} # {element_hash: Prog::Instance} for inputs/textareas
			@dom_onclick_function_handlers = {} # {handler_hash: Prog::Func}
			@route_functions_by_route_name = {} # {route: Prog::Route}

			@load_standard_library = true
			@serve_in_foreground   = true # Hot_Reloader flips this -- see #run
			@input                 = [] # [Prog::Expression]
			@stack                 = [] # [Prog::Scope]
			@servers               = [] # [Prog::Server]
			...
			# Interpreter, so the token the /_backend/live-reload stream reports changes, which is exactly
			...
			@lexer               = Lexer.new
			@parser              = Parser.new
			@declarations        = {} # {::String => Prog::Declaration}, see Declarator
			@forced_declarations = ::Set.new # identity-tracked Prog::Expression, see #resolve_forward_declaration
		...
		def run source_code
			...
			if @stack.empty?
				...
				if load_standard_library
					...
					@stdlib_scope = Prog::Scope.new('Standard_Library')
		...
		def hoistable_declaration_expr? expr
			...
			# `Ident := @load 'file'` / `IDENT := @load 'file'` -- a named load builds a scope of its own, same declarative category as `This := That {}` above. Only a Capitalized/UPPERCASE left-hand name opts in, matching Backend's own casing convention for a namespace-like binding -- a lowercase `mod := @load 'file'` stays a plain variable, not hoisted
			load_call_expr?(expr.right) &&
				%i(Identifier IDENTIFIER).include?(Backend.type_of_identifier(expr.left.value))
		end
		...
		def load_call_expr? expr
			return false unless expr.is_a?(Prog::Call_Expr)
			r = expr.receiver
			r.is_a?(Prog::Infix_Expr) && r.operator&.value == '.' &&
				r.left.is_a?(Prog::Identifier_Expr) && r.left.value == Prog::CONTEXT_OPERATOR &&
				r.right.is_a?(Prog::Identifier_Expr) && r.right.value == 'load'
		end
		...
		def loop_servers
			begin
				...
				trap_fn = Proc.new do
					puts Prog::Ascii.dim "Shutting down..."
					keep_running = false
					puts "\n\s\s(V) (;,,;) (V)"
					Thread.main.exit
				end
				...
				while keep_running
					@servers.each do |server|
						puts "Backend Server `#{server.name}` started at http://localhost:#{server.port}"
		...
		def load_file_into_scope filepath, into_scope
			...
			Backend.assert pop_scope.equal? into_scope
		...
		def scope_for_identifier expr
			unless expr.is_a? Prog::Identifier_Expr
				return stack.last
			end
			...
			when 'self' # the enclosing instance
				...
				stack.reverse_each do |scope|
					if scope.has?(expr.value) || scope.respond_to?("proxy_#{expr.value}")
						found_scope = scope
						break
					elsif scope.is_a?(Prog::Instance) && scope.enclosing_scope&.has?(expr.value)
		...
		def maybe_instance expr
			# todo, when String and so on, because everything needs to be some type of scope to live inside the runtime. Every object in Prog::Scope.declarations{} is either a primitive like String, Integer, Float, or they're an instanced version like Prog::Number.
			case expr
			when ::Integer, ::Float, ::BigDecimal
				# Prog::Number_Expr is already handled in #interpret but this is short-circuiting that for cases like 1.something where we have to make sure the 1 is no longer a numeric literal, but instead a runtime object version of the number 1. The Ruby class of the already-evaluated value picks the matching Backend numeric type (Ruby's own Integer/Float/Rational tower, minus Rational for now). `Integer`/`Float` are bare here (not `::`) on purpose -- they mean `Prog::Integer`/`Prog::Float`.
				prog_class, type_name = case expr
				when ::Integer then [Prog::Integer, 'Integer']
				when ::Float then [Prog::Float, 'Float']
				when ::BigDecimal then [Prog::Decimal, 'Decimal']
				end

				finish_intrinsic_instance prog_class.new(expr), type_name
			when ::String
				finish_intrinsic_instance Prog::String.new(expr), 'String'
			when ::Array
				finish_intrinsic_instance Prog::Array.new(expr), 'Array'
			when ::Hash
				finish_intrinsic_instance Prog::Dictionary.new(expr), 'Dictionary'
			when Prog::Set
				finish_intrinsic_instance expr, 'Set'
			when Prog::Range
				finish_intrinsic_instance expr, 'Range'
			when true
				finish_intrinsic_instance Prog::Bool.truthy, 'Bool'
			when false
				finish_intrinsic_instance Prog::Bool.falsy, 'Bool'
			when nil
				adopt_type Prog::Nil.new, 'Nil'
			else
				expr
			end
		end
		...
		def context_types_for scope
			return wrap_prog_array(scope.type_objects) if scope.is_a?(Prog::Struct)
			return wrap_prog_array(scope.enum_types) if scope.is_a?(Prog::Enum)
			wrap_prog_array(scope.respond_to?(:types) && scope.types ? scope.types.to_a : [])
		end

		def context_type_for scope
			return scope.type_objects&.first if scope.is_a?(Prog::Struct)
			return scope.enum_type if scope.is_a?(Prog::Enum)
			(scope.types&.first if scope.respond_to?(:types)) || scope.class.name.split('::').last
		end

		def context_values_for scope
			return wrap_prog_array(scope.enum_values) if scope.is_a?(Prog::Enum)
		...
		def track_static_declaration scope, ident_expr
			return unless ident_expr.is_a?(Prog::Identifier_Expr) && ident_expr.scope_operator&.value == 'Self'
			scope.static_declarations ||= ::Set.new
			scope.static_declarations.add ident_expr.value.to_s
		end

		def current_instance
			stack.reverse_each.find { |scope| scope.is_a? Prog::Instance }
		end

		def check_dot_access_permissions! scope, ident, expr
			binding = Backend.binding_of_ident scope, ident
			privacy = Backend.privacy_of_ident ident

			case scope
			when Prog::Instance
				if privacy == :private && !current_instance.equal?(scope)
					raise Prog::Cannot_Call_Private_Instance_Member.new(expr)
				end
			when Prog::Type
				if binding == :instance
					...
					#       :1:1
					raise Prog::Cannot_Call_Instance_Member_On_Type.new(expr)
				elsif privacy == :private
					raise Prog::Cannot_Call_Private_Static_Member_On_Type.new(expr)
				end
			end
		end
		...
		def find_ruby_class_for_type type
			candidates = type.types.filter_map do |type_name|
				prog_name = "Prog::#{type_name}"
				next unless Object.const_defined? prog_name
				k = Object.const_get prog_name
				k if k.is_a?(Class) && k < Prog::Instance && k != Prog::Instance
			end

			# Most-derived wins: `Integer | Number {}` matches both Prog::Integer and Prog::Number, and
			# we want Prog::Integer (its `value=` coerces via `to_i`). Longest ancestor chain = deepest
		...
		def type_name_to_string value
			case value
			when Prog::Integer then 'Integer'
			when Prog::Float then 'Float'
			when Prog::Decimal then 'Decimal'
			when Prog::Number then 'Number'
			when ::Integer then 'Integer'
			when ::Float then 'Float'
			when ::BigDecimal then 'Decimal'
			when Prog::String then 'String'
			when Prog::Array then 'Array'
			when Prog::Range then 'Range'
			when Prog::Dictionary then 'Dictionary'
			when Prog::Bool then 'Bool'
			when Prog::Instance then value.types.first
			when Prog::Type then value.name
		...
		# If `name` is already an Prog::Func_Signature, return it as-is (an inline signature has no name to look up). Otherwise, if it's bound to one anywhere on the stack, return that. Otherwise nil — meaning `name` is an ordinary nominal type name (e.g. 'Number').
		def resolve_func_signature name
			return name if name.is_a? Prog::Func_Signature
			return nil unless name
			value = find_in_stack name
			value.is_a?(Prog::Func_Signature) ? value : nil
		end
		...
		def start_server server
			...
			webrick.mount_proc '/onclick/' do |req, res|
				puts Prog::Ascii.dim "#{'DOM'.rjust(7, ' ')} #{req.path}"
			...
			# response access.
			if live_reload
				webrick.mount_proc '/_backend/live-reload' do |_req, res|
					res.status           = 200
		...
		def handle_request server, request, response
			...
			# }`, both in this debug log line and in `request.body` as seen by every Backend route/onclick
			...
			if path_string.start_with?("/onclick/")
				...
				if entry
					begin
						...
						if component.is_a?(Prog::Instance) && component.declarations['render']
						...
						warn "\n[Backend Onclick Error] #{e.class}: #{e.message}"
			...
			if route_function
				...
				begin
					...
					warn "\n[Backend Server Error] #{e.class}: #{e.message}"
					warn e.backtrace.first(10).map { |line| "  #{line}" }.join("\n")
					warn ""
		...
		def build_prog_request path_string, http_method, body_hash, query_params, url_params, headers_hash
			...
			params_dict  = Prog::Dictionary.new url_params
			headers_dict = Prog::Dictionary.new headers_hash
		...
		def render_dom_to_html dom_instance, render_scope: nil
			...
			renderer = Backend::Dom_Renderer.new dom_instance, inner_html
		...
		def append_dom_child_html value, html, render_scope
			...
			when Prog::Array
				...
				html << render_dom_to_html(value, render_scope: render_scope) if value.is_a?(Prog::Instance) && value.types.include?('Dom')
			end
		end
		...
		def raise_missing_scope_operator_target! expr, scope_operator_value
			...
			when 'self'
				raise Prog::Cannot_Use_Instance_Scope_Operator_Outside_Instance.new(expr)
			when 'Self'
				raise Prog::Cannot_Use_Type_Scope_Operator_Outside_Type.new(expr)
			end
		end
		...
		# silently drop its closure the moment it's passed to a Backend-implemented HOF (map/filter/find,
		...
		def rebind_func_to_scope value, scope
			return value unless value.is_a?(Prog::Func) && value.enclosing_scope.instance_of?(Prog::Type)
			func                 = value.dup
		...
		def assign_context_member dot_expr, word_ident, value
			...
			type     = receiver.is_a?(Prog::Instance) ? receiver.enclosing_scope : receiver
		...
		def context_declaration_expr? expr
			return true if expr.is_a?(Prog::Identifier_Expr) && expr.prefixed_with_at && expr.type
			expr.is_a?(Prog::Infix_Expr) && %w(:= =).include?(expr.operator&.value) &&
				expr.left.is_a?(Prog::Identifier_Expr) && expr.left.prefixed_with_at
		end

		# The directive-flagged `@word` on the RHS of a `.` write target, if any.
		def dot_target_at_word dot_expr
			right = dot_expr.right
			right if right.is_a?(Prog::Identifier_Expr) && right.prefixed_with_at
		end
		...
		def interp_identifier expr
			if expr.prefixed_with_at
				...
				candidates = [stack.last, *stack.select { |s| s.is_a?(Prog::Type) }.reverse, global]
				seen       = []
				...
				raise Prog::Undeclared_Identifier.new(expr)
			end
			...
			when 'true'
				...
				return true
			when 'false'
				# todo; return Prog::Bool.falsy
				return false
			when 'Self'
				found = stack.reverse_each.find { |scope| scope.instance_of? Prog::Type }
			...
			value = if scope.is_a? ::Array
				...
				if scope.has?(expr.value) && !scope.respond_to?(proxy_method)
					...
					return rebind_func_to_scope(result, scope) if result.is_a? Prog::Func
					result
					...
					type_def       = if scope.enclosing_scope.is_a?(Prog::Type) && scope.enclosing_scope.has?(expr.value)
					...
					if declared_value.is_a? Prog::Func
				...
				elsif scope.is_a?(Prog::Instance) && scope.enclosing_scope&.is_a?(Prog::Type) && scope.enclosing_scope&.has?(expr.value)
		...
		def interp_string expr
			...
			if interpolation_char_count == 1
				...
				sub_exprs.each do |sub|
					# Reuses the interpreter's own @parser (not a fresh Backend.parse) so it still knows about @operator declarations registered elsewhere in the program -- #input= resets the cursor but not @custom_infix/etc.
		...
		def assign_subscript target, value
			if target.expression.expressions.count > 1
				raise Prog::Too_Many_Subscript_Expressions.new(target)
			end
		...
		def interp_infix_assignment expr
			...
			has_type_annotation = (expr.left.is_a?(Prog::Identifier_Expr) && (expr.left.type || expr.left.tag)) ||
			                      expr.left.is_a?(Prog::Func_Signature_Expr)
			assignment_scope    ||= stack.last if has_type_annotation

			# If using a scope operator but the scope doesn't exist, raise an error
			if expr.left.is_a?(Prog::Identifier_Expr) && expr.left.scope_operator && assignment_scope.nil?
			...
			if expr.left.is_a?(Prog::Identifier_Expr) && !expr.left.scope_operator
				...
				if (current_scope.is_a?(Prog::Instance) || current_scope.is_a?(Prog::Type)) &&
			...
			if expr.left.is_a? Prog::Subscript_Expr
				return assign_subscript expr.left, interpret(expr.right)
			end

			# Handle dot assignment
			if expr.left.is_a?(Prog::Infix_Expr) && expr.left.operator.value == '.'
			...
			if load_call_expr?(expr.right)
				...
				new_scope = Prog::Scope.new expr.left.value
			...
			is_class_declaration = Backend.type_of_identifier(expr.left.value) == :Identifier && right_value.is_a?(Prog::Scope)
			...
			case Backend.type_of_identifier expr.left.value
			when :IDENTIFIER
				...
				if assignment_scope&.has? expr.left.value
					raise Prog::Cannot_Reassign_Constant.new(expr.left)
				end
			when :Identifier
				# It can only be assigned `value` of Prog::Scope, which includes Prog::Type
				if !right_value.is_a?(Prog::Scope)
					raise Prog::Cannot_Assign_Incompatible_Type.new(expr)
				end
			when :identifier
				if assignment_scope
					...
					type      = if expr.left.is_a? Prog::Func_Signature_Expr
					...
					if signature
						unless signature.matches? right_value
							raise Prog::Type_Contract_Violation.new(expr, signature.to_s, describe_value_shape(right_value))
						end
					else
			...
			unless assignment_scope && (assignment_scope.has?(expr.left.value) || has_type_annotation || is_class_declaration)
				...
				raise Prog::Cannot_Assign_Undeclared_Identifier.new(expr)
			end
		...
		def interp_infix_declaration expr
			...
			if expr.left.is_a?(Prog::Circumfix_Expr) && expr.left.grouping == '()'
			...
			if expr.left.is_a? Prog::Subscript_Expr
				raise Prog::Cannot_Declare_Subscript_Target.new(expr)
			end
			...
			has_scope_operator = expr.left.is_a?(Prog::Identifier_Expr) && expr.left.scope_operator
			...
			if has_scope_operator && assignment_scope.is_a?(Prog::Type) && !assignment_scope.has?(expr.left.value)
				raise Prog::Cannot_Assign_Undeclared_Identifier.new(expr) unless still_under_construction? assignment_scope
			end
		...
		def interp_destructuring_declaration expr
			...
			unless targets.all? { |target| destructuring_target? target }
				raise Prog::Invalid_Destructuring_Target.new(expr)
			end
			...
			unless values
				raise Prog::Invalid_Destructuring_Source.new(expr)
			end

			if targets.length > values.length
				raise Prog::Destructuring_Arity_Mismatch.new(expr, targets.length, values.length)
			end

			targets.each_with_index do |target, i|
				value = values[i]

				if target.is_a? Prog::Identifier_Expr
		...
		def destructuring_target? target
			target.is_a?(Prog::Identifier_Expr) ||
				(target.is_a?(Prog::Infix_Expr) && target.operator&.value == '.' && target.right.is_a?(Prog::Identifier_Expr))
		end
		...
		def static_var_declaration_expr? expr
			return false unless expr.is_a?(Prog::Infix_Expr) && expr.operator&.value == ':='

			left = expr.left
			left.is_a?(Prog::Infix_Expr) && left.operator&.value == '.' &&
				left.left.is_a?(Prog::Identifier_Expr) && !left.left.scope_operator && left.left.value == 'Self'
		end
		...
		def assign_dot_member expr, target, value, declare: false
			...
			# self-declared-but-unset member (`x,`) comes back as Prog::Nil. Either way `x.foo = 1` has no
			...
			if receiver.nil? || (receiver.is_a?(Prog::Nil) && !receiver.has?(property))
				raise Prog::Receiver_Is_Nil.new(target)
			end
			...
			# handles that below.
			if property == 'tag' && receiver.is_a?(Prog::Scope) && receiver.has?('tag')
				new_tag               = tag_struct_for_reassignment value, target
				unless tag_chains_satisfy? receiver.tag_instance, new_tag
					raise Prog::Tag_Signature_Violation.new(expr, tag_display_name(receiver), stringify_for_display(new_tag))
				end
			...
			self_keyword = target.left.is_a?(Prog::Identifier_Expr) && !target.left.scope_operator &&
			               Prog::SCOPE_KEYWORDS.include?(target.left.value)

			if self_keyword && receiver.is_a?(Prog::Scope)
				...
				unless receiver.has?(property) || still_under_construction?(receiver) || (declare && receiver.is_a?(Prog::Global))
					raise Prog::Cannot_Assign_Undeclared_Identifier.new(expr)
				end
			...
			unless receiver.is_a?(Prog::Scope) && receiver.has?(property)
				raise Prog::Cannot_Assign_Undeclared_Identifier.new(expr)
			end

			if Backend.type_of_identifier(property) == :IDENTIFIER
				raise Prog::Cannot_Reassign_Constant.new(expr)
			end
		...
		# Prog::Tuple/Prog::Struct both carry a plain Ruby-level `.values` reader holding the raw proxy array (distinct from their Backend-level `.values` dot-access, which wraps the same data in an Prog::Array for Backend code to read).
		def destructurable_values value
			case value
			when Prog::Tuple, Prog::Struct
				value.values
			end
		end
		...
		def interp_dot_infix expr
			...
			unless receiver.kind_of?(Prog::Scope)
				raise Prog::Invalid_Dot_Infix_Left_Operand.new(expr)
			end
			...
			when Prog::Dictionary
				...
				if expr.right.instance_of? Prog::Type_Expr
					return interp_member_access receiver, expr.right
				end

				interp_dot_scope receiver, expr
			end
		rescue Prog::Undeclared_Identifier, Prog::Cannot_Call_Instance_Member_On_Type, Prog::Receiver_Is_Nil, Prog::Invalid_Dot_Infix_Left_Operand
		...
		def stringify_for_display value, show_quotes: false, pretty_print: false
			...
			to_s_ident.lexeme = Prog::Lexeme.new(:identifier, method_name)
			func              = begin
				interp_member_access value, to_s_ident
			rescue Prog::Undeclared_Identifier
				nil
			end
			return value unless func.is_a? Prog::Func
		...
		def interp_member_access receiver, expr, exclude_global_scope: false
			...
			if expr.is_a?(Prog::Identifier_Expr) && !expr.scope_operator && Prog::SCOPE_KEYWORDS.include?(expr.value)
				return receiver[expr.value] if receiver.is_a?(Prog::Scope) && receiver.has?(expr.value)
				raise Prog::Undeclared_Identifier.new(expr)
			end
		...
		def interp_dot_string str, expr
			case
			when expr.right.is(Prog::Number_Expr)
				array_index_value str, expr.right.value, expr
			when expr.right.is(Prog::Array_Index_Expr)
				expr.right.indices_in_order.reduce(str) do |current, index|
					raise Prog::Invalid_Dot_Infix_Left_Operand.new(expr) unless current.is_a?(Prog::Array)
		...
		def interp_dot_array_or_tuple scope, expr
			case
			when expr.right.is(Prog::Func_Expr) && expr.right.name.value == 'each'
		...
		def interp_dot_range range, expr
			return interp_each_loop range, expr.right if expr.right.is(Prog::Func_Expr) && expr.right.name.value == 'each'
			interp_dot_scope range, expr
		end

		def interp_dot_dictionary dict, expr
			if expr.right.is_a? Prog::Identifier_Expr
		...
		def interp_dot_scope scope, expr
			raise Prog::Invalid_Dot_Infix_Left_Operand.new(expr) if scope.nil?
			raise Prog::Invalid_Dot_Infix_Right_Operand.new(expr.right) unless expr.right.instance_of? Prog::Identifier_Expr
			...
			raise Prog::Receiver_Is_Nil.new(expr) if scope.is_a?(Prog::Nil)
			raise
		end

		def interp_each_loop collection, func_expr
			collection.each do |it|
				each_scope                 = Prog::Scope.new 'each(;)'
		...
		def interp_nil_init expr
			...
			if (current_scope.is_a?(Prog::Instance) || current_scope.is_a?(Prog::Type)) && !current_scope.has?(expr.left.value)
			...
			rescue # Prog::Undeclared_Identifier and ArgumentError # todo: Why `ArgumentError: empty string`. Once this is resolved, then the rescue here should explicitly catch Undeclared_Identifier, probably.
		...
		def find_operator_overload operator, operand = nil
			if operand.is_a?(Prog::Instance) && operand.has?(operator)
				return operand.get operator
			end

			if operand.is_a?(Prog::Scope) && operand.enclosing_scope.is_a?(Prog::Type) && operand.enclosing_scope.has?(operator)
				return operand.enclosing_scope.get operator
			end

			stack.reverse_each do |scope|
				next if scope.is_a?(Prog::Type)
		...
		def interp_infix expr
			...
			if (operator == '=' || operator == ':=') && expr.left.is_a?(Prog::Identifier_Expr) && expr.left.prefixed_with_at
		...
		def interp_logical_infix expr
			...
			return call_operator_overload(overload, expr, nil) if overload.is_a? Prog::Func
		...
		def string_value_and_bare_type a, b
			str_of    = ->(v) { v.is_a?(Prog::String) ? v.value : (v.is_a?(::String) ? v : nil) }
			bare_type = ->(v) { v.is_a?(Prog::Type) && !v.is_a?(Prog::Instance) }
		...
		def interp_comparison_infix expr, left, right
			...
			case expr.operator.value
			when '===', '=!=', '=>=', '=<=', '=/='
				# `.type_objects`, not `.types` -- `Prog::Struct < Instance < Type` inherits Type's own
				...
				left_tag  = left.is_a?(Prog::Type) ? left.tag_instance&.type_objects : nil
				right_tag = right.is_a?(Prog::Type) ? right.tag_instance&.type_objects : nil

				# note; `left`/`right` are whatever #interpret returned (a raw Ruby Integer/String/etc for literals, not necessarily an Prog::Type/Instance), so `.types` can't be called on them directly. Using #composed_types_for here which resolves the correct composed-type set.
		...
		def interp_compound_infix expr, left, right
			overload = find_operator_overload expr.operator.value
			return call_operator_overload(overload, expr, [left, right]) if overload.is_a? Prog::Func
		...
		def interp_range_infix expr, start, finish
			overload = find_operator_overload expr.operator.value
			return call_operator_overload(overload, expr, [start, finish]) if overload.is_a? Prog::Func
			...
			finish_intrinsic_instance Prog::Range.new(::Range.new(from, to, exclude_end)), 'Range'
		end
		...
		def interp_custom_infix expr, left, right
			overload = find_operator_overload expr.operator.value, maybe_instance(left)
			unless overload.is_a? Prog::Func
				raise Prog::Undeclared_Infix_Operator.new(expr)
			end
		...
		# @param expr [Prog::Percent_Literal_Expr < Prog::Circumfix_Expr]
		def interp_percent_literal expr
			literal_expr_class = case expr.kind
			when 'string', 'str', 'Str', 'STR' then Prog::String_Expr
			when 'symbol', 'sym', 'Sym', 'SYM' then Prog::Symbol_Expr
			end
			...
			array_expr             = Prog::Circumfix_Expr.new
			array_expr.grouping    = '[]'
			array_expr.expressions = expr.expressions.map do |it|
				# A backtick item is evaluated immediately, like string interpolation, then folded through the same to_s + casing treatment as every other item. No Prog::Statement gets built here (unlike #invoke_statement's callers), so use_caller_scope/memoize never come into play -- it's always immediate, in whatever scope this literal is written in.
				if it.is_a? Prog::Statement_Expr
		...
		def interp_circumfix expr
			case expr.grouping
			when '[]'
				array             = Prog::Array.new
				array.expressions = expr.expressions

				values = []
				expr.expressions.each do |e|
					# Same as #interp_percent_literal above: `` `expr` `` inside an array literal evaluates immediately, no Prog::Statement built.
					e = e.expression if e.is_a? Prog::Statement_Expr
					values << interpret(e)
				end
				link_instance_to_type array, 'Array'

				# Make values accessible as an Backend identifier (`for values`, `arr.values`), sharing the same list object as the real backing store so mutations (push/pop/etc) stay in sync.
				array.values                 = values
			...
			when '()'
				if expr.expressions.count == 1
					...
					tuple  = Prog::Tuple.new values
					link_instance_to_type tuple, 'Tuple'
					tuple.declarations['values'] = tuple.values
					tuple
				end
			when '{}'
				dict = expr.expressions.reduce(Prog::Dictionary.new) do |dict, it|
					if it.is_a? Prog::Identifier_Expr
						dict.proxy_set it.value.to_sym, nil
					elsif it.is_a? Prog::Infix_Expr
						case it.operator.value
						when ':', '='
							if it.left.is_a?(Prog::Identifier_Expr) || it.left.is_a?(Prog::Symbol_Expr) || it.left.is_a?(Prog::String_Expr)
								# note; Deliberately NOT wrap_string_literal_value here, unlike Array/Tuple literals -- Dictionary#hash is handed straight to Ruby-level consumers as a raw Hash (Sequel queries in table.rb chief among them), so wrapping a value into Prog::String here broke every DB call passing string attributes. #to_s below just always double-quotes String values instead of matching the original literal's quote char.
								...
								raise Prog::Invalid_Dictionary_Key.new(it)
							end
						else
							raise Prog::Invalid_Dictionary_Infix_Operator.new(it)
						end
					end
				...
				raise Prog::Unknown_Circumfix_Grouping.new(expr)
			end
		end

		# @param expr [Prog::Call_Expr]
		def interp_call expr
			# A bare `` `expr`() `` written and called in the same place -- always immediate, in whatever scope it's written in. No Prog::Statement is ever built here, so #invoke_statement (used below, once one *has* been built and stored) doesn't apply.
			if expr.receiver.is_a? Prog::Statement_Expr
			...
			if receiver.nil? && expr.receiver.is_a?(Prog::Infix_Expr) && expr.receiver.operator&.value == '.?'
				return nil
			end
			...
			when Prog::Statement
				# Reached once a Statement has been stored in a variable (or field, etc.) and is being called from somewhere else -- Prog::Statement < Instance, so this has to come before the generic Instance branch below or it'd be mistaken for "construct a new Statement".
				invoke_statement receiver

			when Prog::Instance, Prog::Type
				interp_type_call receiver, expr

			when Prog::Func_Signature
				raise Prog::Cannot_Call_Func_Signature.new expr

			else
				raise Prog::Receiver_Is_Not_Callable.new expr.receiver
			end
		end

		# @param expr [Prog::Type_Expr]
		def interp_type expr
			...
			if expr.expressions.nil?
				if expr.tag
					...
					lookup_name = aliased.is_a?(Prog::Type) ? aliased.name : expr.name
					...
					if !existing.is_a?(Prog::Type) && expr.tag.is_a?(Prog::Struct_Expr) && expr.tag.types.length == 1 && expr.tag.names[0].nil?
						...
						if spread_existing.is_a? Prog::Type
							supplied = spread_supplied
							existing = spread_existing
						end
					end
					unless existing.is_a? Prog::Type
						...
						redeclaring_same_struct = aliased.is_a?(Prog::Struct) && aliased.name == expr.name && aliased.structure_declaration_equal?(supplied)
				...
				if expr.tag
					...
					declaration_names      = declaration.is_a?(Prog::Struct) ? declaration.names : []
					declaration_types      = declaration.is_a?(Prog::Struct) ? declaration.type_objects : [] # declared type objects, used below only to detect an unfilled default via identity
					declaration_type_names = declaration.is_a?(Prog::Struct) ? declaration.type_names : []
					declaration_values     = declaration.is_a?(Prog::Struct) ? declaration.values : []
		...
		def interp_anonymous_composition expr
			anonymous             = Prog::Type.new nil
			anonymous.types       = ::Set.new # Type#initialize seeds `@types = ::Set[name]` -- ::Set[nil] here, which would leave a stray nil in .types (breaking #find_ruby_class_for_type's `"Prog::#{type_name}"` lookup) since the union step below only ever adds, never resets.
			...
			push_then_pop anonymous do
				...
				if expr.name
					seed            = Prog::Composition_Expr.new
					seed.operator   = Prog::Lexeme.new(:operator, '|')
					seed.identifier = Prog::Identifier_Expr.new.tap { |it| it.lexeme = Prog::Lexeme.new(:Identifier, expr.name) }
		...
		# Shared tail of both declaration paths below: parent the type to the declaring scope, link it to its Prog:: Ruby class when one exists, record its own name in @types, and run `body_expressions` in the type's scope.
		def finish_type_declaration type, body_expressions
			type.enclosing_scope = stack.last

			prog_name = "Prog::#{type.name}"
			...
			begin
				push_then_pop type do
					body_expressions.each do |sub_expr|
						...
						sub_expr.is_a?(Prog::Composition_Expr) ? interp_composition(sub_expr) : interpret(sub_expr)
					end
				end
			ensure
		...
		def interp_bare_type_declaration expr
			...
			type = (existing.is_a?(Prog::Type) && !existing.is_a?(Prog::Instance)) ? existing : Prog::Type.new(expr.name)
		...
		def resolve_tag_node tag_node, allow_spread: true
			struct = if tag_node.is_a? Prog::Struct_Expr
				...
				when Prog::Type
					...
					raise Prog::Tag_Reference_Must_Be_Type_Or_Struct.new(tag_node)
				end
			end
		...
		def tag_struct_for_reassignment value, node
			return value if value.is_a? Prog::Struct
			return value.tag_instance if value.respond_to?(:tag_instance) && value.tag_instance
			return build_struct [nil], [type_name_to_string(value)], [value], [nil] if value.is_a? Prog::Type
			raise Prog::Tag_Reference_Must_Be_Type_Or_Struct.new(node)
		end
		...
		def declare_tagged_type_variant name, struct, body_expressions
			...
			if existing
				...
				variant             = Prog::Type.new(name)
				blueprint           = stack.last.has?(name) && stack.last[name]
				variant.expressions = blueprint.is_a?(Prog::Type) ? (blueprint.expressions || []).dup : []
			end
		...
		def check_struct_type_contract param, value, expr
			...
			struct.names.each_with_index do |name, i|
				...
				unless value.is_a?(Prog::Scope) && value.has?(name)
					raise Prog::Type_Contract_Violation.new(expr, "<#{struct.names.compact.join(', ')}>", describe_value_shape(value))
				end
				...
				unless candidates.include? declared_type
					raise Prog::Type_Contract_Violation.new(expr, declared_type, inferred_type_name(member_value))
				end
			end
		end
		...
		def check_splat_param_type_contract param, value, expr
			...
			return unless param.type.is_a?(Prog::Identifier_Expr) && param.type.value != 'Any'
			return if type_contract_satisfied?(maybe_instance(value), param.type.value)

			raise Prog::Type_Contract_Violation.new(expr, param.type.value, describe_value_shape(value))
		end
		...
		def member_candidate_type_names value
			...
			when ::TrueClass, ::FalseClass
				...
				if value.is_a?(Prog::Type) && value.types && !value.types.empty?
					value.types.to_a
				else
					[value.name]
				end
		...
		def tagged_variants_for base_name, current_scope_only: false
			...
			scopes.each do |scope|
				# `stack` can briefly hold non-Scope receivers during dot-access (e.g. Prog::Range).
		...
		# For Prog::Database#proxy_create_table: finds the Table-composed type declared with this exact schema, if any, so the created table can be tagged with the model's real identity. Nil if none matches.
		...
		# Searches the full scope stack (innermost to outermost) for `key`, the same way a bare identifier resolves via #scope_for_identifier -- checking only `stack.last` would miss a type declared in an outer/global scope while evaluating from inside a nested context (e.g. a type's own declaration body during composition). This returns an Backend type. `excluding:` skips scopes of that class -- used by #find_operator_overload to keep looking past a currently-executing Type/Instance body, since merely being on the stack doesn't mean the *current* operands belong to it (Instance < Type, so excluding: Prog::Type skips both).
		...
		#
		# Prog::Type_Expr is converted to Prog::Type in #interp_type.
		# Prog::Instance inherits Prog::Type's @name and @types.
		#
		#     (See types.rb for Prog::Type and Prog::Instance declarations)
		#     (See expressions.rb for Prog::Type_Expr declaration)
		#
		...
		# - For each Prog::Func declared on instance, set `func.enclosing_scope = instance`
		...
		def run_type_body_on_instance type, instance
				...
				push_then_pop type do
					push_then_pop instance do |scope|
						type.expressions.each do |expr|
							...
							if expr.is_a?(Prog::Func_Expr) && expr.name.is_a?(Prog::Identifier_Expr) &&
							   expr.name.scope_operator&.value == 'Self'
								next
							end

							# Same bypass as #finish_type_declaration's body walk, re-run per instance here.
							expr.is_a?(Prog::Composition_Expr) ? interp_composition(expr) : interpret(expr)
						end
					end
				end
			end
			...
			instance.declarations.each do |key, decl|
				next unless decl.is_a? Prog::Func
		...
		# Builds the raw instance for #interp_type_call: backed by its Prog:: Ruby class when one exists, linked to its type, struct bound, and the type's body run on it. `Self(;)` is invoked afterward by #interp_type_call itself.
		def build_instance_of_type type, expr
			ruby_class = find_ruby_class_for_type type
			instance   = ruby_class ? ruby_class.new : Prog::Instance.new(type.name)
			...
			# (`Tasks | Table {}` -> Prog::Table baked in "Table"); `@.name` must report "Tasks".
		...
		def interp_type_call type, expr
			...
			if func_new
				...
				raise Prog::Arguments_Given_But_Not_Expected.new(expr)
			end
		...
		def dom_type? type
			type.is_a?(Prog::Type) && type.types.include?('Dom')
		end

		def dom_constructor_prop_name? name
			Prog::DOM_CONSTRUCTOR_PROP_NAMES.include?(name) ||
				Prog::DOM_CONSTRUCTOR_PROP_PREFIXES.any? { |prefix| name.start_with? prefix }
		end

		# @return [Hash{::String => Prog::Expression}] whitelisted `name := value` arguments keyed by
		...
		def interp_func expr
			func                 = Prog::Func.new expr.lexeme
		...
		def interp_func_body func, expr, arg_values: nil
			...
			raise Prog::Invalid_Parameter_Name.new(expr, nameless_param.type.value) if nameless_param
			...
			arg_values ||= begin
				...
				expr.arguments.each do |arg|
					...
					if seen_named && kind != :named
						raise Prog::Positional_Argument_After_Named.new(expr)
					end

					if kind == :named
						seen_named = true
						raise Prog::Duplicate_Named_Argument.new(expr, name_or_label) if named_args.key? name_or_label
			...
			call_scope                 = Prog::Func.new func.name
			...
			if func.enclosing_scope.is_a?(Prog::Instance) && func.enclosing_scope.enclosing_scope
			...
			unless named_args.empty? || has_variadic
				...
				raise Prog::Unknown_Named_Argument.new(expr, unknown_name) if unknown_name
			end
			...
			func.parameters.each_with_index do |param, i|
				...
				if param.variadic
					...
					if named_args.key? name_key # `rest := <value>` at the call site
						...
						raise Prog::Type_Contract_Violation.new(expr, 'Arguments', inferred_type_name(nv)) unless nv.is_a? Prog::Array
				...
				if has_positional && has_named
					raise Prog::Argument_Given_By_Name_And_Position.new(expr, name_key)
				end
				...
				value = if has_named
					...
					raise Prog::Missing_Argument.new(expr)
				end
				...
				if !has_named && supplied_label && supplied_label != param.label&.value
					raise Prog::Argument_Label_Mismatch.new(expr, param.label&.value, supplied_label)
				end

				check_struct_type_contract param, value, expr if param.type.is_a?(Prog::Struct_Expr)
			...
			if call_scope.name == 'assert'
				raise Prog::Assert_Triggered.new(expr) unless interpret(body.first) == true # Just to be explicit.
			end

			result = nil
			body.compact.each do |e|
				result = interpret e
				break if result.is_a? Prog::Return
			end

			Backend.assert pop_scope == call_scope
			Backend.assert pop_scope == func.enclosing_scope

			if func.enclosing_scope.is_a?(Prog::Instance) && func.enclosing_scope.enclosing_scope
				type = func.enclosing_scope.enclosing_scope
				Backend.assert pop_scope == type
				pop_scope if type.enclosing_scope # Pop the Type's enclosing scope
			end

			return_value = result.is_a?(Prog::Return) ? result.value : result
		...
		def classify_argument arg
			if arg.is_a?(Prog::Infix_Expr) && arg.operator&.value == ':=' && arg.left.is_a?(Prog::Identifier_Expr)
				[:named, arg.left.value, arg.right]
			elsif arg.is_a?(Prog::Infix_Expr) && arg.operator&.value == ':' && arg.left.is_a?(Prog::Identifier_Expr)
		...
		def interp_route expr
			...
			enclosing_type = stack.reverse.find do |scope|
				scope.is_a? Prog::Type # note: You could have an instance on the stack, or an empty scope, whatever.
			end
			if enclosing_type
		...
		# @param route [Prog::Route] The route (or onclick handler wrapper) to execute
		# @param req [Prog::Request] Request object to inject
		# @param res [Prog::Response] Response object to inject
		# @param url_params [Hash] Extracted URL parameters (e.g., {"id" => "123"})
		# @param server_instance [Prog::Instance] The server instance (for accessing instance variables)
		...
		def interp_route_body route, req, res, url_params = {}, server_instance: nil
			...
			call_scope = Prog::Scope.new "#{handler.name || 'anonymous'}_route"
			...
			params.each do |param|
				...
				if value.nil?
					if route.param_names.include? param.name.value
						...
						raise Prog::Route_Param_Expected_But_Not_Found.new(route)
					end
			...
			elsif result.is_a? Prog::Array
				html = ''
				result.values.each do |it|
					if it.is_a? ::String
						html += it
					elsif it.is_a?(Prog::Instance) && it.types.include?('Dom')
						html += render_dom_to_html it
					end
				end
				res.declarations['body'] = html
			elsif result.is_a? Prog::Instance
			...
			Backend.assert popped_call == call_scope

			if server_instance
				popped_instance = pop_scope
				Backend.assert popped_instance == server_instance
			end

			popped_enclosing = pop_scope
			Backend.assert popped_enclosing == handler.enclosing_scope
		...
		def interp_statement expr
			instance = Prog::Statement.new expr.expression
			# Capture the scope this literal was built in -- see Prog::Statement's class comment.
		...
		# Enforces use_caller_scope/memoize for an already-built Prog::Statement (#interp_call's `Prog::Statement` branch). Immediate `` `expr`() `` and backtick items in percent/array literals never build a real Statement, so they skip this entirely.
		...
		# @param expr [Prog::Fence_Expr]
		def interp_fence expr
			# `expr.value` is the fence's body wrapped in a String_Expr, not yet interpreted -- passing
			# it straight to Prog::Fence.Self (as this used to) stored the raw AST node as the fence's
			...
			finish_intrinsic_instance Prog::Fence.new(interpret(expr.value)), 'Fence' # note: Prog::Fence extends Prog::String
		end

		# @param expr [Prog::Html_Fence_Expr]
		...
		def interp_composition expr
			...
			unless right.is_a? Prog::Scope
				raise Prog::Invalid_Composition_With_A_Non_Scope_type.new(right)
			end
			...
			when '^'
				...
				raise Prog::Invalid_Composition_Operator.new(expr)
			end
		end
		...
		def interp_struct_composition expr
			...
			expr.expressions.each do |composition_expr|
				...
				when '^'
					...
					raise Prog::Invalid_Composition_Operator.new(composition_expr)
				end
			end
		...
		# @param for_loop_expr [Prog::For_Loop_Expr]
		def interp_for_loop for_loop_expr
			stride = interpret(for_loop_expr.stride) if for_loop_expr.stride

			Backend.assert stride.nil? || stride.is_a?(::Integer), "Stride must be an integer" if stride

			loop_type = for_loop_expr.type&.value || 'each' # one of Prog::FOR_VERBS
			result    = nil
			...
			iterate_body = -> (element, index) do
				...
				begin
					...
					if collection.is_a? Prog::Dictionary
						...
						for_loop_expr.body.each do |e|
							...
							throw(:stop, body_result) if body_result.is_a? Prog::Return
						end
					end
				ensure
					pop_scope
			...
			collected = []
			count_val = 0
			elements  = if stride && !collection.is_a?(Prog::Dictionary)
				# `each_slice` yields raw Ruby Arrays -- wrap each chunk as a real Prog::Array so `it` behaves like any other Backend value (`==`, `.push`, etc.), not just dot-index access (`it.0`), which already worked because #interp_dot_infix calls #maybe_instance on its receiver regardless.
				values.each_slice(stride).map { |chunk| Prog::Array.new(chunk) }.each_with_index
			...
			if elements.equal? collection
				...
				begin
					...
					for_loop_body_result = catch :stop do
						...
						while true
							...
							break if result.is_a?(Prog::Instance) && result.name == 'Done'
							...
							iteration += 1
								...
								for_loop_expr.body.each do |e|
									...
									throw(:stop, for_loop_body_result) if for_loop_body_result.is_a? Prog::Return
								end
							end
						end
					end
				...
				when 'map', 'select', 'reject'
					result = Prog::Array.new(collected)
					link_instance_to_type result, 'Array'
				when 'count'
					result = count_val
				end

				result     = stop_value if stop_value.is_a? Prog::Return
			end
		...
		def interp_conditional expr
			...
			when 'while', 'until', 'elwhile', 'elswhile'
				...
				if expr.when_false.is_a? Prog::Conditional_Expr
				...
				if body.is_a? Prog::Conditional_Expr
		...
		def interp_intrinsic name, args, at = nil, receiver = nil
			...
			when 'to_s'
				...
				label   = subject && (subject.name.is_a?(Prog::Lexeme) ? subject.name.value : subject.name)
			...
			when 'assert'
				raise Prog::Assert_Triggered.new(at, args[1]) unless truthy? args.first
				args.first
			when 'refute'
				raise Prog::Refute_Triggered.new(at, args[1]) if truthy? args.first
				args.first
			when 'connect'
				require 'sequel'
				database = args.first
				...
				unless database.get 'connection'
					url = database.get('url') or raise Prog::Url_Not_Set_For_Database_Instance
			...
			when 'start_server'
				...
				server.port   = Integer(server.get(:port) || Prog::Server::DEFAULT_PORT)
		...
		def interp_ruby_proxy expr
			...
			raise Prog::Invalid_Ruby_Proxy_Usage.new(func_scope) unless func_scope.is_a? Prog::Func
			...
			target = if instance_or_type.instance_of?(Prog::Type) && instance_or_type.name
			...
			if proxy_method && !target.respond_to?(proxy_method)
				raise Prog::Missing_Ruby_Proxy_Declaration.new target, expr
			end

			result = target.send proxy_method, *func_scope.arguments

			# A Ruby-built instance (`Prog::String.new` in a proxy) seeds `@types` from `self.class.name` -- re-wire its type identity so `===`/return-type checks pass.
			adopt_type result, result.class.name.split('::').last if result.is_a?(Prog::Instance) && result.enclosing_scope.nil?
		...
		def interp_context_function func, call_expr
			...
			if Prog::Context::STACK_FUNCTIONS.include? word
		...
		def interp_context_stack_function word, arg_exprs, at
			...
			when 'declare'
				...
				if data.first.is_a? Prog::Struct
				...
				else raise Prog::Invalid_Context_Function_Usage.new(at)
				end
			...
			when 'push_scope'
				...
				raise Prog::Invalid_Scope_Function_Argument.new(target_expr) unless target_expr.is_a?(Prog::Identifier_Expr)
				target = maybe_instance(interpret target_expr)
				raise Prog::Invalid_Scope_Function_Argument.new(target_expr) unless target.is_a?(Prog::Type)
				push_scope target

			when 'pop_scope'
				target_expr = arg_exprs.first
				raise Prog::Invalid_Context_Function_Usage.new(at) unless target_expr
				raise Prog::Invalid_Scope_Function_Argument.new(target_expr) unless target_expr.is_a?(Prog::Identifier_Expr)
				scope_to_pop = maybe_instance interpret target_expr
				raise Prog::Invalid_Scope_Function_Argument.new(target_expr) unless scope_to_pop.is_a?(Prog::Type)
				# By identity -- an instance and a reference to its type don't interchange.
				Backend.assert pop_scope == scope_to_pop
				scope_to_pop

			when 'splat', 'splatr'
				target = maybe_instance interpret(arg_exprs.first)
				raise Prog::Invalid_Context_Function_Usage.new(at) unless target
				raise Prog::Invalid_Scope_Function_Argument.new(arg_exprs.first) unless target.is_a?(Prog::Scope)
				word == 'splatr' ? stack.last.add_readable_scope(target) : stack.last.add_writable_scope(target)
				target

			when 'unsplat'
				raise Prog::Invalid_Context_Function_Usage.new(at) if arg_exprs.empty?
				target = maybe_instance interpret arg_exprs.first
		...
		def interp_subscript expr
			if expr.expression.expressions.count > 1
				raise Prog::Too_Many_Subscript_Expressions.new(expr.expression)
			end
			...
			case receiver
			when Prog::Dictionary, Prog::Array
				key = interpret subscript
				# `arr[1..3]` -- a Prog::Range slice. Unwrap to the proxy ::Range; the result is a raw
				# Ruby sub-array (or nil for an out-of-bounds start), so re-link it.
				if key.is_a?(Prog::Range)
			...
			when Prog::String
				...
				index = index.range if index.is_a?(Prog::Range) # `"abc"[0..<2]`
				receiver.value[index]
			else
				raise Prog::Invalid_Subscript_Receiver.new expr.receiver
			end
		end
		...
		# Builds (but doesn't declare) a real Prog::Enum for an Enum_Expr -- shared by #interp_enum and nested enum members (#build_enum_member).
		def build_enum expr
			# Named at construction, not via `.name =` after -- a later attr write never touches @declarations.
			instance = Prog::Enum.new expr.name.value
			link_instance_to_type instance, 'Enum'

			# Skips normal Type-construction, so Enum's own Backend-level body (keys/values/types/count, @operator ==) is run by hand.
			type = instance.enclosing_scope
			if type
				instance.expressions = type.expressions
				run_type_body_on_instance type, instance
		...
		# Links a raw Prog::Array to the real Array type so its own Backend-level methods (to_s(;), etc.) are reachable. Used by #build_enum and #build_instance_of_type.
		...
		def interp_type_annotation type_expr
			return interp_struct type_expr if type_expr.is_a? Prog::Struct_Expr
			return interpret type_expr unless type_expr.tag

			reference      = Prog::Type_Expr.new
		...
		def interp_struct expr, allow_spread: true
			...
			expr.types.each_with_index do |member, i|
				if expr.names[i]
					...
					if member.is_a?(Prog::Func_Signature_Expr)
					...
					if single_member && value.is_a?(Prog::Struct)
						...
						values << (value.is_a?(Prog::Type) && !value.is_a?(Prog::Instance) ? nil : wrap_string_literal_value(member, value))
						names << nil
					end
				end
			end
		...
		def predeclare_bare_named_struct_stub expr
			return nil unless expr.name.is_a? Prog::Lexeme
			...
			return existing if existing.is_a?(Prog::Struct) && existing.names.empty?
			return nil unless existing.nil? && tagged_variants_for(name).empty?

			stub       = Prog::Struct.new
		...
		def register_bare_named_struct name, struct, expr, stub = nil
			...
			if existing.is_a?(Prog::Struct) && existing.name == name && existing.structure_declaration_equal?(struct)
				return existing
			end
		...
		# The low-level Prog::Struct object is always built first and exactly the same way regardless of `struct_type` -- it's what #type_objects/etc. read from, and every existing member-matching call site depends on it being real.
		def build_struct names, type_names, types, values
			struct_type = find_in_stack 'Struct'
			struct      = Prog::Struct.new names, type_names, types, values

			unless struct_type.is_a?(Prog::Type)
			...
			if member_type.is_a?(Prog::Type)
				members = names.each_index.map do |i|
					...
					member              = Prog::Member.new names[i], member_display_type, values[i]
					link_instance_to_type member, 'Member'
					member
				end

				members_array = Prog::Array.new members
		...
		# @param expr [Prog::Operator_Overload_Expr]
		...
		def interpret expr
			case expr
			when Prog::Number_Expr, Prog::Symbol_Expr
				expr.value

			when Prog::Identifier_Expr
				interp_identifier expr

			when Prog::String_Expr
				interp_string expr

			when Prog::Type_Expr
				interp_type expr

			when Prog::Route_Expr
				interp_route expr
			...
			when Prog::Composition_Expr
				...
				raise Prog::Composition_Outside_Type_Declaration.new(expr)

			when Prog::Prefix_Expr
				interp_prefix expr
			...
			when Prog::Postfix_Expr
				interp_postfix expr

			when Prog::Percent_Literal_Expr
				interp_percent_literal expr

			when Prog::Circumfix_Expr
				interp_circumfix expr

			when Prog::Call_Expr
				interp_call expr

			when Prog::For_Loop_Expr
				interp_for_loop expr

			when Prog::Conditional_Expr
				interp_conditional expr

			when Prog::Array_Index_Expr
				maybe_instance expr.indices_in_order

			when Prog::Subscript_Expr
				interp_subscript expr

			when Prog::Statement_Expr
				interp_statement expr

			when Prog::Fence_Expr
				interp_fence expr

			when Prog::Html_Fence_Expr
				interp_html_fence expr

			when Prog::Comment_Expr
				expr.value

			when Prog::Operator_Overload_Expr
				interp_operator_overload expr

			when Prog::Operator_Expr
				case expr.value
				when 'skip'
					throw :skip
				when 'stop'
			...
			when nil
				...
				raise Prog::Interpret_Expr_Not_Implemented.new(expr)
			end
		end
	end
end
