module Backend
	class Parser
		...
		# If the given operator doesn't exist then it returns Prog::DEFAULT_OPERATOR_PRECEDENCE which binds somewhere around the equality operators. See Prog::PRECEDENCES
		...
		def curr? * sequence
			...
			slice.each_with_index.all? do |lexeme, index|
				...
				if expected.is_a?(Prog::Array) || expected.is_a?(::Array)
					expected.any? do |alt|
						lexeme.is(alt)
					end
				else
		...
		def func_declaration_follows?
			...
			remainder.each do |token|
				...
				return true if token.value == Prog::FUNCTION_DELIMITER && depth == 1
		...
		def anon_func_param_list_follows?
			...
			remainder.each do |token|
				...
				return true if token.value == Prog::FUNCTION_DELIMITER
		...
		def try_parse_struct_body
			saved_i = @i
			start   = curr_lexeme
			Prog::Struct_Expr.new.tap do |it|
				it.lexeme = Prog::Lexeme.new :struct, '<>'
				it.types  = []
				it.names  = []
				eat '<'
		...
		def parse_for_loop_expr
			it            = Prog::For_Loop_Expr.new
			it.lexeme     = eat 'for'
			it.collection = parse_expression

			if curr? Prog::FOR_VERBS and verb = eat
				it.type   = verb
				it.lexeme = verb
			end
		...
		def parse_enum_expr
			...
			until curr? ']'
				...
				item = if curr? TYPE_IDENTIFIER, '['
					...
					Backend.assert item_name.is_a? Prog::Identifier_Expr
		...
		def parse_func precedence = STARTING_PRECEDENCE
			...
			until curr? Prog::FUNCTION_DELIMITER
				before_i = @i
				...
				if curr? TYPE_IDENTIFIER
					...
					if curr?('...') and eat('...') # f (x...; ...) -- same as f (x: Arguments; ...)
						param.variadic = true
						param.type     ||= Prog::Identifier_Expr.new('Arguments')
					end
			...
			eat Prog::FUNCTION_DELIMITER if curr? Prog::FUNCTION_DELIMITER
			reduce_newlines
		...
		def parse_struct_members it
			...
			begin
				loop do
					...
					elsif element.is_a?(Prog::Identifier_Expr) && element.type && curr?('=')
						eat '='
						element.member_default = parse_expression(precedence_for('<'))
					end
					...
					raise Prog::Invalid_Struct_Member_Annotation.new(curr_lexeme) if curr? ':'

					it.types << element
					it.names << if element.is_a?(Prog::Func_Signature_Expr)
						element.name&.value || element.value
					elsif element.is_a?(Prog::Identifier_Expr) && (element.type || element.member_default)
						element.value
					end
		...
		def parse_type_decl
			...
			start        = curr_lexeme
			it           = Prog::Type_Expr.new eat # one of valid_idents
			...
			Backend.assert is_type || is_const, "Type names can only be Capitalized or UPPERCASE" # todo; proper error
		...
		def parse_comment
			lexeme   = eat
			it       = Prog::Comment_Expr.new lexeme
			it.value = Prog::String_Expr.new lexeme
			it.body  = Prog::String_Expr.new lexeme
			it.type  = lexeme.type
			it
		end
		...
		def parse_html_expr
			...
			start      = curr_lexeme
			it         = Prog::Html_Fence_Expr.new eat
			it.value   = Prog::String_Expr.new start
			it.body    = it.value
		...
		def parse_composition_expr
			...
			if ident.is_a?(Prog::Identifier_Expr) && ident.tag
				type_ref      = Prog::Type_Expr.new
		...
		def parse_statement_expr
			Prog::Statement_Expr.new.tap do |it|
				eat '`'
		...
		def parse_self_prefixed_identifier
			...
			expr.scope_operator = Prog::Lexeme.new(:operator, keyword.value)
			expr
		end
		...
		def parse_route_expr
			...
			handler_params = if expr.is_a? Prog::Func_Expr
			...
			route             = Prog::Route_Expr.new
			route.http_method = Prog::Identifier_Expr.new.tap do |expr|
		...
		def parse_percent_literal_expr
			...
			percent_lit             = Prog::Percent_Literal_Expr.new # This extends Circumfix_Expr
			...
			valid_items = percent_lit.expressions.all? do |it|
				...
				it.is_a?(Prog::Identifier_Expr) || it.is_a?(Prog::Number_Expr) || it.is_a?(Prog::Operator_Expr) || it.is_a?(Prog::Statement_Expr)
			end

			raise Prog::Invalid_Percent_Literal_Expression.new(percent_lit) unless valid_items
		...
		def parse_percent_literal_item
			...
			return item if item.is_a? Prog::Statement_Expr
		...
		def merge_percent_literal_items left, right
			...
			merged               = Prog::Identifier_Expr.new lexeme
		...
		def parse_beginless_range_expr
			Prog::Infix_Expr.new.tap do |it|
				it.left     = nil
				it.operator = eat
				it.right    = parse_expression precedence_for(it.operator.value)
				it.right    = it.right.left if it.right.is_a? Prog::Nil_Init_Expr
		...
		def parse_operator_expr
			...
			it = Prog::Operator_Expr.new operator_lexeme
			copy_location it, start
		end

		def parse_number_expr
			start       = curr_lexeme
			expr        = Prog::Number_Expr.new start
			expr.lexeme = eat(:number)
			if expr.lexeme.value.count('.') > 1
				expr                  = Prog::Array_Index_Expr.new expr.lexeme
		...
		def begin_expression precedence = STARTING_PRECEDENCE, member_rhs: false
			raise Prog::Out_Of_Tokens.new unless lexemes?

			if curr? :route
				parse_route_expr

			elsif curr?(ANY_IDENTIFIER, Prog::NIL_INIT_POSTFIX) || curr?(SCOPE_KEYWORDS, '.', ANY_IDENTIFIER, Prog::NIL_INIT_POSTFIX)
				parse_nil_init_expr
				...
				# No trailing `{` guard needed for the named-reference form -- unlike `/`, `\` never collides with anything else in Backend, so it's unambiguous with or without a body.
				parse_type_decl
				...
				start = curr_lexeme
				expr  = Prog::String_Expr.new eat(:string)
				copy_location expr, start
				...
				raise Prog::Reserved_Function_Delimiter.new curr_lexeme
		...
		def parse_context_call context_ident, precedence
			member          = Prog::Infix_Expr.new
			member.operator = Prog::Lexeme.new(:operator, '.')
			member.left     = Prog::Identifier_Expr.new(Prog::CONTEXT_OPERATOR)
			member.right    = Prog::Identifier_Expr.new(context_ident.value)
			...
			bare_ref = !paren && !bare_arg && !Prog::Context::STACK_FUNCTIONS.include?(context_ident.value)
		...
		def parse_operator_overload op_ident, precedence
			...
			operator_expr = Prog::Operator_Expr.new op_lexeme
			copy_location operator_expr, op_lexeme

			next_expr = begin_expression
			if next_expr.is_a?(Prog::Identifier_Expr) && next_expr.prefixed_with_at && %w(prefix infix postfix circumfix).include?(next_expr.value)
				prec = if curr? '('
				...
				overload            = Prog::Operator_Overload_Expr.new operator_expr.lexeme
		...
		def complete_expression expr, precedence = STARTING_PRECEDENCE, member_rhs: false
			...
			if !member_rhs && expr.is_a?(Prog::Identifier_Expr) && expr.prefixed_with_at
				...
				unless context_member_decl
					...
					return parse_context_call(expr, precedence) if Prog::Context::FUNCTIONS.include?(expr.value)
				end
			end
			...
			prefix    = !expr.is_a?(Prog::Operator_Overload_Expr) && !expr.is_a?(Prog::String_Expr) && (PREFIX.include?(expr.value) || (expr.is_a?(Prog::Operator_Expr) && @custom_prefix.include?(expr.value)))
			...
			if prefix
				expr = Prog::Prefix_Expr.new.tap do |it|
					it.operator   = expr
				...
				if COMPOUND_OPERATORS.include? curr_lexeme.value
					...
					while (INFIX.include?(curr_lexeme.value) || @custom_infix.include?(curr_lexeme.value)) && curr?(:operator)
						...
						# It's very important that the curr?(:operator) check here remains because otherwise it breaks Prog::Call_Expr when the receiver is an Prog::Infix_Expr.
						...
						if expr.left.is(Prog::Identifier_Expr) && expr.operator.value == '.' && expr.right.is(Prog::Number_Expr) && expr.right.type == :float
							...
							number                  = Prog::Array_Index_Expr.new expr.right
				...
				expr = Prog::Postfix_Expr.new.tap do |it|
					it.expression = expr
			...
			spread_receiver   = expr.is_a?(Prog::Call_Expr) || expr.is_a?(Prog::Subscript_Expr) || (expr.is_a?(Prog::Infix_Expr) && expr.operator&.value == '.')
