...
module Prog
	class Set < Instance
		...
		def to_ruby_set other
			...
			return other.set.dup         if other.is_a? Prog::Set
			...
			Backend.assert false, "Set expected an Array, Range, or Set, got #{other.class.name.split('::').last}"
		end
		...
		def | other
			Prog::Set.new.tap { |s| s.set.replace(@set | to_ruby_set(other)) }
		end
		...
		def == other
			other.is_a?(Prog::Set) && @set == other.set
		end
