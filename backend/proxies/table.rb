module Prog
	class Table < Instance
		...
		def proxy_all
			...
			Prog::Array.new records
		end

		def proxy_find id
			id = id.to_i if id.is_a?(::String)
			Backend.assert id.is_a?(::Numeric), "is actually #{id.inspect}"
		...
		def proxy_where struct
			...
			Prog::Array.new rows.map { |row| row_to_struct(row) }
		end
		...
		def proxy_update id, struct
			Backend.assert id.is_a? ::Numeric
			Backend.assert struct.is_a? Prog::Struct
		...
		def row_to_struct row
			...
			struct = Backend::Interpreter.current.build_struct columns.names, columns.type_names, columns.type_objects, values
		...
		def coerce_column_value raw, type_name
			...
			when 'Date'
				raw.nil? ? nil : linked_temporal(Prog::Date.new(raw.is_a?(::Date) ? raw : ::Date.parse(raw.to_s)), 'Date')
			when 'Time'
				raw.nil? ? nil : linked_temporal(Prog::Time.new(raw.is_a?(::Time) ? raw : ::Time.parse(raw.to_s)), 'Time')
			when 'Date_Time'
				raw.nil? ? nil : linked_temporal(Prog::Date_Time.new(raw.is_a?(::DateTime) ? raw : ::DateTime.parse(raw.to_s)), 'Date_Time')
			else
				raw
			end
		end
		...
		def linked_temporal instance, type_name
			Backend::Interpreter.current&.link_instance_to_type instance, type_name
			instance
		end

		def table
			raise Prog::Database_Not_Set_For_Table_Instance unless database
		...
		def check_filter_columns! struct
			...
			raise Prog::Table_Invalid_Filter_Column,
