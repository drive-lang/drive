	...
	class Struct < Instance
		...
		def []= key, value
			...
			if @names && (i = @names.index(key.to_s))
				@values[i] = value
				if (member = @members&.values&.at(i)).is_a?(Prog::Member)
					member.value      = value
					member['value']   = value
				end
			end
		...
		# to use directly: an Prog::String yields its raw ::String, and a raw ::Symbol (an enum
		...
		def to_h
			names.each_with_index.each_with_object({}) do |(name, i), hash|
				...
				when Prog::String then value.value
				when Prog::Bool then value.truthiness
				when Prog::Date, Prog::Time, Prog::Date_Time then value.value
