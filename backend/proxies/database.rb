...
module Prog
	class Database < Instance
		...
		def table_name_for struct
			Backend.assert struct.name, "table_name_for expects a named struct, got an anonymous one"
			pluralize(underscore(struct.name)).to_sym
		end

		def proxy_find_or_create_table struct
			Backend.assert struct.name
		...
		# @param [Prog::Struct] struct with name
		# @return [Prog::Table] table
		def proxy_create_table struct
			Backend.assert struct.is_a? Prog::Struct
			Backend.assert struct.name

			# It appears that #create_table here doesn't return anything so below this block, I'm forwarding to #find_table which actually builds a Prog::Table
			connection.create_table table_name_for(struct) do
				...
				struct.members.values.each do |member|
					...
					when 'Time'
						...
						# note; no Backend types yet for these below
					when 'Flo', 'Float'
		...
		def find_table_named name
			Backend.assert name.is_a? ::String
		...
		def find_table_struct struct
			Backend.assert struct.name, "#find_table_struct expects the given struct to be declared with a name."
			table         = find_table_named table_name_for(struct).to_s
			table.columns = struct
			table
		end

		# @param [::Symbol, Prog::Struct] name_or_struct a table name, or a named schema struct to derive one from
		def proxy_delete_table! name_or_struct
			name = name_or_struct.is_a?(Prog::Struct) ? table_name_for(name_or_struct) : name_or_struct
		...
		def proxy_tables
			Prog::Array.new connection.tables
		end
