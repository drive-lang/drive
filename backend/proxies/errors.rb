...
module Prog
	...
	class Undeclared_Identifier < Error
		# `expression` is whatever AST node was being resolved when lookup failed, usually an Prog::Identifier_Expr (a plain identifier reference), but also an Prog::Type_Expr when a bare type reference (`Abc()`) never got declared. Both expose `.value` via the shared Expression base (set from their own lexeme), so one implementation covers either raise site without caring which one it actually got.
		def detail_message
	...
	class Undeclared_Tagged_Type < Error
		...
		# @expression: Prog::Type_Expr | Prog::Struct_Expr

		def detail_message
			if expression.is_a? Prog::Struct_Expr
				name      = expression.name.is_a?(Prog::Lexeme) ? expression.name.value : expression.name
				...
				# bare named struct can't reuse the name (rename it, the way backend/expressions.prog's
	...
	# @expression: Prog::Type_Expr
	class Tag_Reference_Must_Be_Type_Or_Struct < Error
		def detail_message
			label = expression.respond_to?(:name) ? "#{expression.name}#{Prog::TAG_OPERATOR}#{expression.tag&.value}" : expression.value
			"`#{Ascii.bold label}` -- the right-hand side of `#{Prog::TAG_OPERATOR}` must resolve to a Struct or a Type, not a plain value."
		end
	end
	...
	class Receiver_Is_Nil < Error
		# `expression` is the `.`/`.?` Prog::Infix_Expr whose left side evaluated to nil, for both reads
	...
	class Invalid_Percent_Literal_Expression < Error
		def detail_message
			...
			expected = exprs.map do |it|
				'Prog::Identifier_Expr or Prog::Number_Expr or Prog::Operator_Expr'
			end.join(', ')
