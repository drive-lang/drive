module Prog
	# note: Be sure to prefix with Prog:: whenever referencing this Array type to prevent ambiguity with Ruby's ::Array!
	class Array < Instance
		...
		def proxy_Self *args
			...
			source =
				if args.length == 1
					case (a = args.first)
					when Prog::Array, Prog::Set then a.values
					when Prog::Range            then a.range.to_a
					when ::Array                then a
		...
		def proxy_flatten depth = -1
			ruby_array = values.map { |v| v.is_a?(Prog::Array) ? v.values : v }
			Prog::Array.new ruby_array.flatten depth
		end
		...
		def proxy_reverse
			Prog::Array.new values.reverse
		end

		def proxy_sort
			Prog::Array.new values.sort
		end

		def proxy_uniq
			Prog::Array.new values.uniq
		end
		...
		def + other
			Prog::Array.new(values + other.values)
		end

		private

		def wrap_if_array result
			result.is_a?(::Array) ? Prog::Array.new(result) : result
		end
	end

	class Tuple < Prog::Array
		def initialize values = []
			super values
		end
