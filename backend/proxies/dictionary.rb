module Prog
	class Dictionary < Instance
		...
		def proxy_keys
			Prog::Array.new hash.keys
		end

		def proxy_values
			Prog::Array.new hash.values
		end

		def proxy_merge other_hash
			Prog::Dictionary.new hash.merge other_hash.hash
		end

		# A key argument reaching any of these five can be a raw Ruby String/Symbol (a literal interpreted directly, e.g. by #interp_infix_assignment's own subscript handling) or a real Prog::String instance (an ordinary call argument, e.g. a variable holding one) -- Prog::String has no #to_sym of its own, so unwrap it to its real Ruby value first either way.
		def normalize_dict_key key
			(key.is_a?(Prog::String) ? key.value : key).to_sym
		end
