...
cd drive
bundle install
bundle exec bin/drive demos/hello_world.disk -p # => Hello, Drive!
```
...
- [`disks`](disks) contains code for the Drive standard library
...
2. **Add an AST node** — a new `Disk::Foo_Expr < Expression` in [`expressions.rb`](drive/backings/expressions.rb). Only add `attr_accessor`s for what's structurally new; `value`/`type`/`l0..c1`/`source_file` are inherited.
...
5. **Interpret it** — add a case to `Interpreter#interpret`'s dispatch and a new `interp_foo` in [`interpreter.rb`](drive/5_interpreter/interpreter.rb) that walks the `Foo_Expr` and produces a runtime value (an `Disk::*` instance, a Ruby primitive, `nil`, etc.).
...
#### Ruby-backed types (`Foo {}` + `Disk::Foo`)

Two files, independently optional — pure-Drive types skip #2, rare Ruby-only types skip #1:

1. **`disks/foo.disk`** — the Drive-level declaration (`Foo { ... }`). A method that defers to Ruby is just `some_method (; @ruby )`.
2. **`drive/backings/foo.rb`** — `class Foo < Disk::Instance` (or `< Disk::Type`), inside `module Drive`. `extend Ruby_Proxies` + `proxy :method_name` for 1:1 delegation ([`ruby_proxies.rb`](drive/shared/ruby_proxies.rb)), or hand-write `def proxy_method_name(...)` for custom logic. `@ruby` calls `proxy_#{method_name}` on the backing instance.
...
4. **Load the Drive file** — `@load 'disks/foo.disk'` in [`disks/global.disk`](disks/global.disk) for always-on, or leave opt-in for the user's own program to `@load` (e.g. `disks/database.disk`).
...
1. `Interpreter#find_ruby_class_for_type(type)` walks `type.types` (most-derived first), returns the first Ruby constant `Disk::#{type_name}` that's a `Class < Disk::Instance`.
2. `Interpreter#build_instance_of_type(type, expr)` calls #1: found → `ruby_class.new`; not found → `Disk::Instance.new(type.name)`.
...
#### Instance/Type without a backing `Disk::Class`

1. Perfectly valid — most user `Type { }`s have no Ruby class; `build_instance_of_type` falls back to plain `Disk::Instance.new(type.name)`.
2. A plain `Disk::Instance` works normally for everything declared in Drive — `declarations` hash, methods, `Self(;)`, composition, structs.
3. Only `@ruby` breaks:
   - Outside a `Func` scope → `Disk::Invalid_Ruby_Proxy_Directive_Usage`.
   - Instance doesn't `respond_to?("proxy_#{method_name}")` (no Ruby class, or Ruby class missing that one `proxy_*` method) → `Disk::Missing_Ruby_Proxy_Declaration`.
4. So: forgetting the Ruby class is safe unless the `.disk` body calls `@ruby` — then it's a runtime error on first call, not at declaration time.
...
| Standard library (`.disk` side of built-ins) | [`disks/`](disks), auto-loaded via [`disks/global.disk`](disks/global.disk) |
