...
# Runs every demos/*.disk file end to end and asserts none of them raise -- these are meant to be
...
class Demos_Test < Base_Test
	Dir.glob(File.join(__dir__, '../demos/*.disk')).sort.each do |filepath|
		name = File.basename(filepath, '.disk')
