...
class Declarator_Test < Base_Test
	def test_top_level_func_is_declared
		decls = Backend.declare "add ( a, b; a + b )"
		assert decls.key? 'add'
	end

	def test_literal_rhs_is_preserved_not_dropped
		decls = Backend.declare "greeting := 'hi'"
		assert_instance_of Prog::String_Expr, decls['greeting'].expr_or_decl
	end

	def test_plain_identifier_rhs_is_not_double_wrapped
		decls = Backend.declare "flag := true"
		assert_instance_of Prog::Identifier_Expr, decls['flag'].expr_or_decl
	end

	def test_named_struct_gets_its_declaring_name
		decls = Backend.declare "Named <String, Number>"
		assert_equal 'Named', decls['Named'].expr.name.value
	end

	def test_conditional_branches_flatten_into_enclosing_scope
		decls = Backend.declare <<~CODE
		    check (;
		    	if true
		    		a := 1
		    	elwhile false
	...
	def test_call_site_named_arguments_are_not_declarations
		decls = Backend.declare "sub(a := 1, b := 2)"
		assert_empty decls
	end
	...
	def test_bare_identifier_reference_does_not_clobber_the_real_declaration
		decls = Backend.declare <<~CODE
		    ...
		    (x)
		CODE
		assert_instance_of Prog::Number_Expr, decls['x'].expr_or_decl
	end
	...
	def test_genuinely_undeclared_identifier_still_raises
		assert_raises Prog::Undeclared_Identifier do
			Backend.interp "totally_missing()"
		end
	end
