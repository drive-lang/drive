(function () {
	...
	document.addEventListener('click', async (event) => {
		const element = event.target.closest('a[href], button, input:not([type="hidden"]), select, textarea, summary, [data-drive-onclick]')

		if (!element) return
		if (!element.hasAttribute('data-drive-onclick')) return
		...
		const target_id = response.headers.get('X-Drive-Target-Id')
