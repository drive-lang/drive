module Drive
	class Parser
		...
		def parse_type_decl
			...
			Drive.assert is_type || is_const, "Type names can only be Capitalized or UPPERCASE" # todo; proper error
		...
		def parse_identifier_expr
			...
			expr.lexeme  = eat
			expr.privacy = Drive.privacy_of_ident expr.value
			...
			expr.kind = Drive.type_of_identifier expr.value
			copy_location expr, start
		end
		...
		def begin_expression precedence = STARTING_PRECEDENCE, member_rhs: false
			...
			if curr? :route
				...
				# No trailing `{` guard needed for the named-reference form -- unlike `/`, `\` never collides with anything else in Drive, so it's unambiguous with or without a body.
				parse_type_decl
