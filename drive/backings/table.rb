module Tape
	class Table < Instance
		...
		def proxy_find id
			...
			Drive.assert id.is_a?(::Numeric), "is actually #{id.inspect}"
		...
		def row_to_struct row
			...
			struct = Drive::Interpreter.current.build_struct columns.names, columns.type_names, columns.type_objects, values
		...
		def linked_temporal instance, type_name
			Drive::Interpreter.current&.link_instance_to_type instance, type_name
			instance
		end
