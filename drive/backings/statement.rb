module Disk
	# Drive wrapper for `*_Expr < Expression`. Built two ways:
	...
	#    sees a throwaway arg (same as every Ruby-backed Drive type, e.g. Disk::String/Disk::Array); real
	#    argument binding happens in disks/statement.disk's `Self(;)` instead.
	class Statement < Instance
		...
		# Disk::Func#enclosing_scope's closure trick. Set only by Interpreter#interp_statement; nil
		# otherwise, and Interpreter#invoke_statement then falls back to use_caller_scope behavior.
		#
		# use_caller_scope/memoize/memoized/_memoized_value live in disks/statement.disk as ordinary Drive
		...
		def proxy_from other
			return unless other.is_a? Disk::Statement
