...
class Temporal_Test < Base_Test
	def test_date_today_is_a_date
		assert_equal Time.now.year, Tape.interp('Date.today().year')
	end

	def test_date_parse_reads_components
		out = Tape.interp('d := Date.parse("2020-03-15"), [d.year, d.month, d.day]')
		assert_equal [2020, 3, 15], out.values
	end

	def test_date_weekday
		# 2020-03-15 is a Sunday
		assert_equal 0, Tape.interp('Date.parse("2020-03-15").weekday')
	end

	def test_date_iso8601
		assert_equal '2020-03-15', Tape.interp('Date.parse("2020-03-15").iso8601()')
	end

	def test_date_comparison
		assert_equal true, Tape.interp('Date.parse("2020-01-01") < Date.parse("2021-01-01")')
		assert_equal false, Tape.interp('Date.parse("2021-01-01") < Date.parse("2020-01-01")')
		assert_equal true, Tape.interp('Date.parse("2020-06-01") == Date.parse("2020-06-01")')
		assert_equal true, Tape.interp('Date.parse("2020-06-02") != Date.parse("2020-06-01")')
	end

	def test_time_now_and_epoch
		assert_equal true, Tape.interp('Time.now().epoch() > 1500000000')
	end
	...
	def test_time_at_epoch_zero
		...
		out = Tape.interp('t := Time.at(0), [t.epoch(), t.year]')
		assert_equal 0, out.values[0]
		assert_includes [1969, 1970], out.values[1]
	end

	def test_date_time_now_components
		out = Tape.interp('dt := Date_Time.now(), [dt.year >= 2026, dt.month >= 1, dt.hour >= 0]')
		assert_equal [true, true, true], out.values
	end

	def test_date_time_parse_and_iso8601
		out = Tape.interp('Date_Time.parse("2020-03-15T09:30:00+00:00").iso8601()')
	...
	def test_date_time_comparison
		code = <<~TAPE
		    ...
		    [a < b, a <= a, b > a, a == a]
		TAPE
		assert_equal [true, true, true, true], Tape.interp(code).values
	end
